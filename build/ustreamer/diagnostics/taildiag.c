...
#define RAW_LIMIT 32
#define RAW_BUDGET (64U * 1024U * 1024U)
#define RECENT 2048
#define LIMIT 8192
#define DISK_BUDGET (64U * 1024U * 1024U)
...
static uint64_t raw_bytes, queued_bytes, raw_skipped[3], write_errors;
static unsigned raw_reserved[3];
...
static FILE *create(const char *name) {
    struct stat st;
    if(!fstatat(directory,name,&st,AT_SYMLINK_NOFOLLOW) || errno!=ENOENT) return NULL;
    char partial[128];snprintf(partial,sizeof(partial),"%s.partial",name);
    int fd=openat(directory,partial,O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC,0600);
    if(fd<0) return NULL;
    FILE *f=fdopen(fd,"wb");if(!f) close(fd);return f;
}
static bool complete(FILE *f, const char *name) {
    bool ok=!ferror(f);
    if(fclose(f)) ok=false;
    char partial[128];snprintf(partial,sizeof(partial),"%s.partial",name);
    if(ok && linkat(directory,partial,directory,name,0)) ok=false;
    if(ok && unlinkat(directory,partial,0)) ok=false;
    if(!ok) ++write_errors;
    return ok;
}
/* Serialized writer consumes a service-UID request containing 32 hex bytes.
 * Snapshot publication precedes acknowledgement; absence is always failure. */
static void flush_requested(void) {
    int fd=openat(directory,"flush.request",O_RDONLY|O_NONBLOCK|O_NOFOLLOW|O_CLOEXEC);
    if(fd<0) return;
    struct stat st;char nonce[33]={0};
    bool valid=fstat(fd,&st)==0 && S_ISREG(st.st_mode) && st.st_uid==geteuid() && st.st_size==32;
    if(valid) valid=read(fd,nonce,32)==32;
    close(fd);
    for(unsigned i=0;valid && i<32;++i)
        if(!((nonce[i]>='0' && nonce[i]<='9') || (nonce[i]>='a' && nonce[i]<='f'))) valid=false;
    if(!valid) return; /* Caller deadline fails; request remains evidence. */
    summary *copy=malloc(sizeof(recent));
    if(!copy) return;
    pthread_mutex_lock(&lock);
    uint64_t cost=RECENT*2048U+8192;
    bool admitted=snapshots<8 && cost<=DISK_BUDGET-reserved_bytes;
    unsigned count=0;
    if(admitted) {
        reserved_bytes+=cost;count=recent_count;
        for(unsigned i=0;i<count;++i) copy[i]=recent[(recent_pos+RECENT-count+i)%RECENT];
    }
    pthread_mutex_unlock(&lock);
    char name[96];snprintf(name,sizeof(name),"recent-%03u.json",++snapshots);
    bool ok=false;
    if(admitted) {
        FILE *f=create(name);
        if(f) {
            fprintf(f,"{\"diagnostic_only\":true,\"nonce\":\"%s\",\"request_realtime\":%lld.%09ld,\"boundaries\":[",
                nonce,(long long)st.st_mtim.tv_sec,st.st_mtim.tv_nsec);
            for(unsigned i=0;i<count;++i) {if(i) fputc(',',f);metadata(f,&copy[i]);}
            fputs("]}\n",f);ok=complete(f,name);
            if(ok) ++snapshots_written;
        } else ++write_errors;
    }
    free(copy);
    /* Only one outstanding request is permitted by the harness. */
    if(unlinkat(directory,"flush.request",0)) return;
    char ack[96];snprintf(ack,sizeof(ack),"flush-%s.json",nonce);
    FILE *f=create(ack);
    if(f) {
        fprintf(f,"{\"nonce\":\"%s\",\"result\":\"%s\",\"snapshot\":\"%s\"}\n",
                nonce,ok?"passed":"failed",name);complete(f,ack);
    } else ++write_errors;
}
    ...
    for(;;) {
        ...
        if(e->save_raw) {
            ...
            if(f) {
                ...
                if(!complete(f,payload)) ok=false;
                saved=ok;
        ...
        if(f) {
            ...
            fprintf(f,",\"metadata_suppressed\":false,\"raw_dump_status\":\"%s\",\"jpeg\":", saved?"saved":(e->save_raw?"write_failed":"quota_exhausted"));us_taildiag_describe(f,e->data,e->row.used);fputs("}\n",f);
            bool ok=complete(f,name);
            if(ok) ++written[e->row.stage];
        }
        pthread_mutex_lock(&lock);queued_bytes-=e->row.used;free(e->data);e->data=NULL;e->ready=false;pthread_mutex_unlock(&lock);
        flush_requested();
    }
    return NULL;
}
static void finish(void) {
    ...
    if(f) {
        ...
        for(unsigned i=0;i<3;++i) fprintf(f,"%s{\"stage\":\"%s\",\"accepted\":%u,\"written\":%u,"
            ...
            "\"suspicious_seen\":%"PRIu64",\"metadata_misses\":%"PRIu64",\"raw_written\":%u,\"raw_skipped\":%"PRIu64"}",
            i?",":"",stages[i],accepted[i],written[i],suppressed[i],oversized[i],inspected[i],suspicious_seen[i],metadata_misses[i],full[i],raw_skipped[i]);
        fprintf(f,"],\"snapshots\":%u,\"snapshots_written\":%u,\"write_errors\":%"PRIu64",\"metadata_reserved_bytes\":%"PRIu64",\"raw_reserved_bytes\":%"PRIu64"}\n",snapshots,snapshots_written,write_errors,reserved_bytes,raw_bytes);complete(f,"summary.json");
    }
    close(directory);
...
static void initialize(void) {
    ...
    fprintf(f,"{\"diagnostic_only\":true,\"pid\":%ld,\"max_payload_bytes\":%u,"
        "\"queue_slots\":%u,\"per_stage_limit\":%u,\"min_interval_seconds\":0,"
        "\"full_payloads_per_stage\":32,\"raw_budget_bytes\":67108864,\"queue_byte_limit\":33554432,\"recent_summaries\":%u,\"disk_budget_bytes\":%u}\n",(long)getpid(),MAX_BYTES,SLOTS,LIMIT,RECENT,DISK_BUDGET);
    if(!complete(f,"session.json")) goto fail;
...
void us_taildiag(unsigned stage,const unsigned char *p,size_t n,size_t allocated,
    ...
    if(s.suspicious) {
        unsigned slot;
        for(slot=0;slot<SLOTS;++slot) if(!queue[slot].ready) break;
        /* Conservative upper bound: all raw EOI offsets, full tail hex, metadata. Raw disk budget is independent. */
        uint64_t eois=0;
        for(size_t j=0;j+1<n;++j) if(p[j]==255 && p[j+1]==217) ++eois;
        uint64_t cost=22*eois+2*(uint64_t)(n-end-2)+8192;
        if(accepted[stage]>=LIMIT || slot==SLOTS || n>QUEUE_BYTES-queued_bytes || cost>DISK_BUDGET-reserved_bytes) ++suppressed[stage];
        else {
            event *e=&queue[slot];
            e->data=malloc(n?n:1);
            if(!e->data) {++suppressed[stage];pthread_mutex_unlock(&lock);goto done;}
            queued_bytes+=n;reserved_bytes+=cost;
            e->save_raw=raw_reserved[stage]<RAW_LIMIT && n<=RAW_BUDGET-raw_bytes;
            if(e->save_raw) {++raw_reserved[stage];raw_bytes+=n;} else ++raw_skipped[stage];
            e->row=s;e->ordinal=++accepted[stage];
            e->prior_count=count;memcpy(e->prior,prior,count*sizeof(summary));
            memcpy(e->data,p,n);e->ready=true;pthread_cond_signal(&wake);
        }
    }
    pthread_mutex_unlock(&lock);
done:
