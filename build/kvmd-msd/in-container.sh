...
root=/work/out/kvmd-msd/$1-rootfs
output=/work/out/kvmd-msd/artifacts
...
test ! -e "$root"
mkdir -p "$root" "$output"
printf '%s  %s\n' "8f7488d12e98662655aa84cc9e4d2bf0920d5a5e1dd2917bb4793288c2a7d663" /work/out/kvmd-hid/artifacts/rootfs.tar.gz | sha256sum -c -
tar --numeric-owner -xpf /work/out/kvmd-hid/artifacts/rootfs.tar.gz -C "$root"
...
if [ "$1" = package ]; then
    ...
    printf '%s  %s\n' "$KVMD_SHA256" /work/out/kvmd-web/downloads/kvmd.tar.gz | sha256sum -c -
    tar -xf /work/out/kvmd-web/downloads/kvmd.tar.gz --strip-components=1 -C "$root/build/source"
    ...
    chroot "$root" /bin/sh -c 'cd /build/source; patch -p1 < /build/video-only.patch'
    printf '%s  %s\n' "$WEB_PATCH_SHA256" /work/build/kvmd-web/web-auth.patch | sha256sum -c -
    cp /work/build/kvmd-web/web-auth.patch "$root/build/web-auth.patch"
    chroot "$root" /bin/sh -c 'cd /build/source; patch -p1 < /build/web-auth.patch'
    cp /work/build/kvmd-hid/hid.patch "$root/build/hid.patch"
    chroot "$root" /bin/sh -c 'cd /build/source; patch -p1 < /build/hid.patch'
    cp /work/build/kvmd-msd/msd.patch "$root/build/msd.patch"
    chroot "$root" /bin/sh -c 'cd /build/source; patch --fuzz=0 -p1 < /build/msd.patch'
    python3 /work/build/kvmd-msd/package.py "$root/build/source" "$root/build/pkg"
    find "$root/build/pkg" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
    dpkg-deb --root-owner-group -Zgzip -z9 --build "$root/build/pkg" "$output/kvmd-web_4.213-1blikvm4_arm64.deb"
else
    cp "$output/kvmd-web_4.213-1blikvm4_arm64.deb" "$root/tmp/kvmd.deb"
    chroot "$root" apt-get install -y --no-install-recommends /tmp/kvmd.deb
    chroot "$root" systemctl enable nginx.service
    chroot "$root" /usr/bin/kvmd --dump-config > "$output/effective-config.yaml"
    sed -i '1i enable blikvm-msd-helper.service\nenable blikvm-gadget.service\nenable nginx.service\nenable kvmd.service\ndisable ustreamer.service' "$root/etc/systemd/system-preset/00-blikvm-lab.preset"
    chroot "$root" dpkg-query -W '-f=${Package}\t${Version}\t${Architecture}\n' > "$output/packages.tsv"
    if test -f /work/build/kvmd-msd/packages.lock.tsv; then cmp /work/build/kvmd-msd/packages.lock.tsv "$output/packages.tsv"; fi
    cp "$output/packages.tsv" "$root/etc/blikvm-packages.tsv"
    printf 'kvmd=%s\nkvmd_commit=%s\n' "4.213-1blikvm4" "$KVMD_COMMIT" >> "$root/etc/blikvm-build"
...
PY
    ...
    cp /work/out/kvmd-lan/artifacts/{Image,sun50i-h616-blikvm-v4.dtb,linux.config} "$output/"
fi
cd "$output"
