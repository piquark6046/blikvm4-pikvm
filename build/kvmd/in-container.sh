...
root=/work/out/kvmd/$1-rootfs
output=/work/out/kvmd/artifacts
...
if [ "$1" = package ]; then
    ...
    python3 /work/build/kvmd/package.py "$root/build/source" "$root/build/pkg"
    find "$root/build/pkg" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
    dpkg-deb --root-owner-group -Zgzip -z9 --build "$root/build/pkg" "$output/kvmd-video_${KVMD_PACKAGE_VERSION}_arm64.deb"
else
    cp "$output/kvmd-video_${KVMD_PACKAGE_VERSION}_arm64.deb" "$root/tmp/kvmd.deb"
    ...
    sed -i '1i enable kvmd.service\ndisable ustreamer.service' "$root/etc/systemd/system-preset/00-blikvm-lab.preset"
    chroot "$root" dpkg-query -W '-f=${Package}\t${Version}\t${Architecture}\n' > "$output/packages.tsv"
    cmp /work/build/kvmd/packages.lock.tsv "$output/packages.tsv"
