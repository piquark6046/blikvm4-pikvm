...
sudo -n docker run --rm --user "$(id -u):$(id -g)" \
  ...
  dest=/work/out/ubuntu/kernel
  cat /work/out/build/artifacts/linux.config /work/build/ubuntu/linux-systemd.config > "$dest/input.config"
  ...
  if ! test -f "$obj/.config-input.sha256" || test "$(cat "$obj/.config-input.sha256")" != "$input_hash"; then
    "$src/scripts/kconfig/merge_config.sh" -m -O "$obj" /work/out/build/artifacts/linux.config /work/build/ubuntu/linux-systemd.config
  ...
  "$src/scripts/diffconfig" /work/out/build/artifacts/linux.config "$dest/linux.config" > "$dest/config-delta.txt"
  cd "$dest"
