#!/usr/bin/env python3
"""Check candidate against frozen M8-C files and exact report encodings."""
import argparse
import ast
import hashlib
import json
...
r=a.root;base=Path('out/kvmd-lan/rootfs');lib=Path('usr/lib/python3/dist-packages/kvmd')
# Every inherited file is frozen unless explicitly reviewed here.
allowed={str(lib/x) for x in ('apps/kvmd/__init__.py','apps/kvmd/server.py','apps/kvmd/api/auth.py','apps/kvmd/api/hid.py',
          'htserver.py','usb.py','plugins/hid/otg/events.py','plugins/hid/otg/__init__.py')}
allowed.update(('usr/share/kvmd/web/share/js/kvm/session.js',))
checked=[]
