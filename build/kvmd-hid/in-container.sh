...
root=/work/out/kvmd-hid/$1-rootfs
output=/work/out/kvmd-hid/artifacts
...
test ! -e "$root"
mkdir -p "$root" "$output"
printf '%s  %s\n' "c79135fa661968323137d2618f035988d20d37c978f9517c504199d15c616ad1" /work/out/kvmd-lan/artifacts/rootfs.tar.gz | sha256sum -c -
tar --numeric-owner -xpf /work/out/kvmd-lan/artifacts/rootfs.tar.gz -C "$root"
...
if [ "$1" = package ]; then
    ...
    python3 /work/build/kvmd-hid/package.py "$root/build/source" "$root/build/pkg"
    find "$root/build/pkg" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
    dpkg-deb --root-owner-group -Zgzip -z9 --build "$root/build/pkg" "$output/kvmd-web_4.213-1blikvm3_arm64.deb"
else
    cp "$output/kvmd-web_4.213-1blikvm3_arm64.deb" "$root/tmp/kvmd.deb"
    ...
    sed -i '1i enable blikvm-gadget.service\nenable nginx.service\nenable kvmd.service\ndisable ustreamer.service' "$root/etc/systemd/system-preset/00-blikvm-lab.preset"
    chroot "$root" dpkg-query -W '-f=${Package}\t${Version}\t${Architecture}\n' > "$output/packages.tsv"
    if test -f /work/build/kvmd-hid/packages.lock.tsv; then cmp /work/build/kvmd-hid/packages.lock.tsv "$output/packages.tsv"; fi
    cp "$output/packages.tsv" "$root/etc/blikvm-packages.tsv"
    printf 'kvmd=%s\nkvmd_commit=%s\n' "4.213-1blikvm3" "$KVMD_COMMIT" >> "$root/etc/blikvm-build"
