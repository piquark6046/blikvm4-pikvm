...
sudo -n docker image inspect "$BUILDER_IMAGE" > out/kvmd-hid/builder-image.json
actual_builder=$(python3 -c 'import json; print(json.load(open("out/kvmd-hid/builder-image.json"))[0]["Id"])')
test "$actual_builder" = "$USTREAMER_BUILDER_ID" || { echo 'Unexpected builder image ID' >&2; exit 1; }
if [ "$mode" = rootfs ]; then
    sha256sum -c build/kvmd-hid/package.sha256
fi
sudo -n docker run --rm --privileged -e "SOURCE_DATE_EPOCH=$SOURCE_DATE_EPOCH" \
  -v "$repo:/work" "$BUILDER_IMAGE" \
  bash -c 'cp /work/build/kvmd-hid/in-container.sh /tmp/ustreamer-build.sh; exec unshare --user --map-users=0,0,65536 --map-groups=0,0,65536 --mount --pid --fork --mount-proc bash /tmp/ustreamer-build.sh "$1"' -- "$mode"
sudo -n chown -R "$(id -u):$(id -g)" out/kvmd-hid/artifacts
