...
root=/work/out/kvmd-web/$1-rootfs
output=/work/out/kvmd-web/artifacts
...
test ! -e "$root"
mkdir -p "$root" "$output"
printf '%s  %s\n' "$M8A_ROOTFS_SHA256" /work/out/kvmd/artifacts/rootfs.tar.gz | sha256sum -c -
tar --numeric-owner -xpf /work/out/kvmd/artifacts/rootfs.tar.gz -C "$root"
...
if [ "$1" = package ]; then
    ...
    python3 /work/build/kvmd-web/package.py "$root/build/source" "$root/build/pkg"
    find "$root/build/pkg" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
    dpkg-deb --root-owner-group -Zgzip -z9 --build "$root/build/pkg" "$output/kvmd-web_${WEB_PACKAGE_VERSION}_arm64.deb"
else
    cp "$output/kvmd-web_${WEB_PACKAGE_VERSION}_arm64.deb" "$root/tmp/kvmd.deb"
    ...
    sed -i '1i enable nginx.service\nenable kvmd.service\ndisable ustreamer.service' "$root/etc/systemd/system-preset/00-blikvm-lab.preset"
    chroot "$root" dpkg-query -W '-f=${Package}\t${Version}\t${Architecture}\n' > "$output/packages.tsv"
    cmp /work/build/kvmd-web/packages.lock.tsv "$output/packages.tsv"
    cp "$output/packages.tsv" "$root/etc/blikvm-packages.tsv"
    printf 'kvmd=%s\nkvmd_commit=%s\n' "$KVMD_PACKAGE_VERSION" "$KVMD_COMMIT" >> "$root/etc/blikvm-build"
