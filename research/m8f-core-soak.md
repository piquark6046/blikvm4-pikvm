...
The user authorized a diagnostic kernel and preparatory restart. M8-F1 observation 01 is now RUNNING from **2026-09-10 07:46:24 UTC**, using exact Run 03 userspace (10,652 live `/usr` file hashes matched) with the memory-reporting kernel delta. Fresh main kvmd PID 889/start_ticks 22446, first HID/MSD cycle and initial two-client video passed. [Start record](evidence/m8f1/diagnostic-kernel/observation-start.json) documents the 12-hour interval, single end restart, 15-minute recovery sampling and automatic private evidence collection. No boundedness/leak outcome or Run 03 acceptance is yet claimed.
