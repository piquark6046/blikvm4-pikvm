...
The actual bootloader was verified to provide Ethernet, `tftpboot`, `dhcp`, and `booti`; its load addresses were read from `printenv` and checked against `bdinfo`. It has no `usb` command, so U-Boot USB storage is not an alternative transport. The target/host Ethernet cable is not currently connected, so the TFTP transfer itself remains the next physical validation.
...
setenv bootargs "console=ttyS0,115200 earlycon loglevel=8 rdinit=/init ip=192.168.77.2::192.168.77.1:255.255.255.0:blikvm:eth0:off"
...
build pinned Linux 7.x + DTB + Alpine initramfs
  ...
  -> wait for initramfs ready marker and SSH
  -> collect dmesg, device probes, tests, and artifact identities
