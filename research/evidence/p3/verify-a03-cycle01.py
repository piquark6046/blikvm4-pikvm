...
def verify(archive,digest):
 ...
 name='p3-a03-cycle-001'
 with tarfile.open(archive) as t:
  read=lambda n:json.load(t.extractfile(n))
  declaration=read('controller/'+name+'/cycle.json');assert 'result' not in declaration and declaration['record']=='immutable_cycle_declaration'
  c={**declaration,**read('controller/'+name+'/cycle-final.json'),**read('controller/'+name+'/pre-reboot.json')}
  c['startup_boot_id']=read('controller/'+name+'/startup-recovered.json')['boot_id']
  c['firmware_markers']=read('controller/'+name+'/firmware-chain.json')['markers']
  assert read('controller/'+name+'/reboot-request.json')['returncode'] in (0,255)
  old=read('controller/'+name+'/preboot/target.json');start=read('controller/'+name+'/startup/target.json')
  assert c['result']=='CYCLE_PASS_PENDING_INDEPENDENT_REPLAY'
  assert c['attempt']==3 and c['cycle']==1 and c['total_cycles']==12 and c['accepted_cycles']==0
  ...
  assert not any(m.name=='controller/P3_A03_FAILED.json' for m in t)
  ...
  assert prov['clean'] is True and prov['commit']==prov['origin_main'] and len(prov['commit'])==40
 ...
 result.update(result='P3_A03_CYCLE_001_ACCEPTED',attempt=3,cycle=1,accepted_cycles=1,total_cycles=12,qualification_credit=1,boot_id=start['boot_id'],preparation_reboot_counted=False,p3='UNACCEPTED',p3_b_c='BLOCKED')
 return result
