...
PROTOCOL=P3['read_json'](Path(__file__).with_name('p3-h3-protocol.json'))['hid']
...
try:
 ...
 with contextlib.ExitStack() as stack:
  ...
  browser=subprocess.Popen(['/usr/bin/python3',str(Path(__file__).with_name('p3-h5r1-functional.py')),'browser','hid'],stdout=log,stderr=subprocess.STDOUT,stdin=subprocess.DEVNULL,close_fds=True)
  index=1;deadline=time.monotonic()+600
  while browser.poll() is None:
   assert time.monotonic()<deadline,'browser overall timeout'
   stem=a.output/f'{index:03d}';ready=LEAF/(stem.name+'.ready')
   if not ready.exists():time.sleep(.03);continue
   spec=P3['read_json'](ready);P3['require'](1 <= index <= len(PROTOCOL) and spec == PROTOCOL[index-1], 'untrusted HID stage');before=[H['drain'](c) for c in caps]
   restart=None
   ...
   assert spec['kind']!='restart','target service repair forbidden'
   P3['publish'](Path(os.environ['P3_CONTROL'])/stem.with_suffix('.go').name,'go',0o644)
   end=time.monotonic()+60
   while not (LEAF/(stem.name+'.done')).exists():
    assert browser.poll() is None,'browser failed before done: '+spec['name']
    assert time.monotonic()<end,'stage timeout: '+spec['name']
    time.sleep(.03)
   details=P3['read_json'](LEAF/(stem.name+'.done'));raw=[H['drain'](c) for c in caps];actual=[H['frames'](r) for r in raw]
   ...
   P3['publish'](Path(os.environ['P3_CONTROL'])/stem.with_suffix('.ok').name,{'result':check['result'],'error':check.get('error')},0o644)
   result['steps'].append({'index':index,'name':spec['name'],'result':check['result']})
   assert check['result']=='passed',spec['name']+': '+check.get('error','')
   index+=1
  result['browser']=P3['read_json'](LEAF/'browser-result.json');assert browser.returncode==0,result['browser'].get('error')
