...
const {chromium}=require('/var/lib/blikvm-p3-h5r1/input/runtime/node_modules/playwright');
...
async function step(name,kind,action,options={}){
 ...
 publish(stem+'.ready',JSON.stringify({name,kind,...options}));await wait(path.join(process.env.P3_CONTROL,path.basename(stem))+'.go');
 sent=[];const details=await action()||{};await pause(250);details.sent=sent;
 publish(stem+'.done',JSON.stringify(details));await wait(path.join(process.env.P3_CONTROL,path.basename(stem))+'.ok');
 const checked=JSON.parse(fs.readFileSync(path.join(process.env.P3_CONTROL,path.basename(stem))+'.ok'));assert.equal(checked.result,'passed',JSON.stringify(checked));
 result.steps.push({name,kind,...details});
}

const launchGate=require('/var/lib/blikvm-p3-h5r1/input/functional/lab/p3-h5r1-launch.cjs');

try {
 launchGate(out);browser=await chromium.launch({headless:false});result.version=browser.version();
