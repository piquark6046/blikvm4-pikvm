#!/usr/bin/env python3
"""M8-B bridge qualification: real MJPEG, source interruption and frozen M5."""
import argparse
...
def main(a):
    ...
    for name in ('web-video-hil.py','web-video-api.py','stream-client.py','hdmi-off.py'):
    ...
    def api(label):
        ...
        p=subprocess.run(command,input=(HERE/'web-video-api.py').read_text(),capture_output=True,text=True,timeout=60)
    ...
    try:
        ...
        if re.search(r'ERROR|Traceback \(most recent call last\)', '\n'.join(line for line in journal.splitlines() if 'Got access denied for user' not in line)): raise RuntimeError('kvmd/streamer journal error')
        monitor.stop()
...
if __name__=='__main__':
    ...
    a=p.parse_args()
    if a.seconds < (15 if a.boot_smoke else 120): p.error('Full HIL requires 120s; boot smoke requires 15s plus the separate 120s HTTPS gate')
