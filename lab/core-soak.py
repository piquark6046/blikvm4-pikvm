...
def main():
    ...
    if a.pilot_seconds is not None and a.pilot_seconds < 900:
        p.error('pilot must last at least 900 seconds')
    root = a.output.resolve(); root.mkdir(parents=True, exist_ok=False)
    seconds = a.pilot_seconds or 86400
    result = {'result': 'failed', 'qualification': 'pilot' if a.pilot_seconds else 'M8-F',
    ...
    def resources(label):
        row = json.loads(target(label, (HERE/'soak-resources.py').read_text()))
    ...
    try:
        ...
        resources('baseline')
        offsets = [120, 300, 480, 660, 780] if a.pilot_seconds else [14400, 28800, 43200, 57600, 72000]
        schedule = list(zip(offsets, ('nginx', 'kvmd', 'logout', 'hdmi', 'nginx')))
