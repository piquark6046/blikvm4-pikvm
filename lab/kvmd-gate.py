#!/usr/bin/env python3
"""Independently audit M8-A video API, native stream and hardware evidence."""
import argparse
import json
...
def audit_api(path, minimum=11):
    api_files=list(path.glob('api-*.json'))
    if len(api_files)<minimum: raise ValueError('missing kvmd API coverage')
    for f in api_files:
        api=json.loads(f.read_text())
        if api['result']!='passed' or len(api['snapshots'])!=12 or len(api['websockets'])!=3:
            raise ValueError('incomplete kvmd API qualification')
        if len({r['sha256'] for r in api['snapshots']})<8:
            raise ValueError('frozen kvmd snapshots')
        if api['saved_state']['snapshot']['saved'] is None or api['state_after']['snapshot']['saved'] is not None:
            raise ValueError('snapshot state did not update')
        for session in api['websockets']:
            if not {'loop','streamer','clients','pong'}.issubset({e['event_type'] for e in session}):
                raise ValueError('incomplete WebSocket session')
            counts=[e['event']['count'] for e in session if e['event_type']=='clients']
            if not counts or any(n!=1 for n in counts): raise ValueError('WebSocket clients leaked between sessions')


def audit_no_signal(raw):
    raw=raw.lstrip()
    state,end=json.JSONDecoder().raw_decode(raw)
    lines=raw[end:].splitlines()
    if not state['result']['streamer']['source']['online']: raise ValueError('MS2131 lost capture availability')
    hashes=[line.split()[0] for line in lines if re.fullmatch(r'[0-9a-f]{64}  -',line)]
    if len(hashes)!=4 or len(set(hashes))!=1: raise ValueError('missing static kvmd no-signal snapshots')


def audit_run(path):
    result=json.loads((path/'test-results.json').read_text())
    if result['result']!='passed' or result['stage']!='kvmd_video_qualification':
    ...
    for i in range(3):
        audit_no_signal((path/f'no-signal-kvmd-{i}.log').read_text())
    journal=(path/'final-journal.log').read_text()
    errors=[line for line in journal.splitlines() if re.search(r'ERROR|Traceback',line)]
    if errors: raise ValueError('uStreamer errors: '+str(errors))
    audit_api(path)
    resource_rows=[]
    for line in (path/'resources.log').read_text().splitlines():
        fields=line.split()
        if len(fields)==8 and fields[0].isdigit() and fields[2] in ('kvmd',) and fields[1]=='1':
