#!/usr/bin/env python3
"""Offline M8-F replay. A pilot or incomplete run can never pass the 24h gate.
...
def duration_gate(result):
    return (result['qualification'] == 'M8-F' and result['required_seconds'] == 86400
            and result['result'] == 'completed_pending_independent_review'
            and result['end_monotonic']-result['start_monotonic'] >= 86400)
...
def audit(root):
    ...
    checks['24_hours'] = duration_gate(r)
    ...
    checks['scheduled_events'] = len(r['events']) == 5 and [e['event'] for e in r['events']] == ['nginx', 'kvmd', 'logout', 'hdmi', 'nginx']
    ...
    checks['logout_denied'] = any(b.get('event') == 'logout-denied' and b['status'] in (401, 403) for b in browser)
    ...
    checks['resource_coverage'] = len(samples) >= 1400 and all(b-a <= 120 for a,b in zip(stamps, stamps[1:]))
