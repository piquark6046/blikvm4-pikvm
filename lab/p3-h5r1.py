#!/usr/bin/env python3
"""Prospective bridge-only H5R1 preparation/minimal controller. Never contacts target.
...
def idle():
    ...
    for p in Path('/proc').glob('[0-9]*/status'):
        try:
            ...
            pass
    require(not found, 'H5R1 UID processes remain: '+repr(found))
    return {'uid': uid, 'pids': found, 'checked_ns': time.time_ns()}


# Archive/seal retain H3's tested whole-leaf rename architecture, using H5R1 UID.
...
def prepare():
    source = source_check()
    require(not BASE.exists(), 'H5R1 namespace exists; no destructive reprepare')
    try:
        pwd.getpwnam(USER)
    except KeyError:
        pass
    else:
        raise RuntimeError('H5R1 account already exists')
    mkdir(BASE, 0o755)
    ...
    for p in [RUNTIME, *RUNTIME.rglob('*')]:
        m = H['no_acl'](p)
        require(m['uid'] == 0 and not m['mode'] & 0o022, 'runtime not immutable to H5R1 UID')
    ...
    command(['/usr/bin/certutil', '-A', '-d', db, '-n', 'H5R1 enrolled public CA', '-t', 'C,,', '-i', str(ca)])
    certs = command(['/usr/bin/certutil', '-L', '-d', db])
    keys = subprocess.run(['/usr/bin/certutil', '-K', '-d', db], capture_output=True, text=True)
    require('H5R1 enrolled public CA' in certs and 'no keys found' in keys.stdout+keys.stderr, 'NSS public-only check failed')
    ...
    contract = {'scope': 'P3-H5R1 prospective runtime', 'uid': u.pw_uid, 'gid': u.pw_gid, 'groups': [u.pw_gid],
    ...
    config['protected_roots'] += ['/var/lib/blikvm-p3-h5', '/home/user/blikvm-p3-h5-exports', '/var/lib/blikvm-p3-h3', '/home/user/blikvm-p3-h3-exports', '/home/user/blikvm-p3-h4-exports']
...
def audit(leaf, destination):
    ...
    p = subprocess.run(['runuser', '-u', USER, '--', '/usr/bin/python3', str(BASE/'input/p3-h5r1.py'), 'worker'],
...
def isolated(leaf, requirements, mode, host_netns):
    ...
    os.execve('/usr/bin/xvfb-run', ['/usr/bin/xvfb-run', *c['xvfb_arguments'], '/usr/bin/node',
               str(BASE/'input/p3-h5r1-blank.cjs')], env)
...
def selftest():
    ...
    try:
        ...
        with (run/'netns.log').open('x') as log:
            p = subprocess.run(['/usr/bin/unshare', '--net', '/usr/bin/python3',
                str(BASE/'input/p3-h5r1.py'), 'isolated', '', str(req), 'selftest',
    ...
    require(r['result'] != 'FAILED', 'H5R1 stopped before Chromium')


def prior_replay(name):
    r = read(BASE/'controller'/name/'result.json')
    ack = read(BASE/'controller'/(name+'-vm-replay.json'))
    require(ack['result'] == 'H5R1_MINIMAL_ACCEPTED' and ack['name'] == name,
...
def minimal(number):
    ...
    try:
        ...
        with (run/'browser-debug.log').open('x') as log:
            p = subprocess.run(['/usr/bin/unshare', '--net', '/usr/bin/python3', str(BASE/'input/p3-h5r1.py'),
        ...
        publish(run/'result.json', r)
    print(json.dumps(r))
    require(r['result'] != 'FAILED', 'H5R1 stopped; no later probe or target contact')
