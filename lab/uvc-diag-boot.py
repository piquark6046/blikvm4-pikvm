#!/usr/bin/env python3
"""M8-F0-only boot: key publication by both kernel and RAM-root hashes."""
import argparse
import hashlib
import json
...
def boot(args):
    ...
    try:
        ...
        # Deliberately retain the M8-A 96 MiB bound at the unchanged RAM load address.
        if manifest['artifacts']['initramfs.cpio.gz']['size'] >= 0x6000000:
            raise RuntimeError('Ubuntu initramfs exceeds M8-A 96 MiB RAM bound')
        recorder.metadata.update(artifacts=manifest['artifacts'], build_source=manifest['source'],
                                 ...
                                 ramdisk_window=dict(start='0x4ff00000', exclusive_end='0x55f00000',
                                                     limit_bytes=0x6000000, qualification='M8-A'))
        recorder.save_text('rootfs-manifest.json', json.dumps(manifest, indent=2))
        publication = args.tftp_root / 'm7' / (manifest['artifacts']['initramfs.cpio.gz']['sha256'][:12] + '-' + manifest['artifacts']['Image']['sha256'][:12])
