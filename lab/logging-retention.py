#!/usr/bin/env python3
"""M8-F2 memory attribution controller derived from the frozen workload.
...
def main():
    ...
    p.add_argument('--observation-seconds', type=int, default=7200)
    p.add_argument('--release-seconds', type=int, default=0)
    a = p.parse_args()
    if a.observation_seconds != 7200 or a.release_seconds != 0:
        p.error('M8-F2 requires exactly 2 hours; no release restart')
    root = a.output.resolve(); root.mkdir(parents=True, exist_ok=False)
    seconds = a.observation_seconds + a.release_seconds
    result = {'result': 'failed', 'qualification': False, 'diagnostic': 'M8-F2', 'observation_seconds': a.observation_seconds, 'release_seconds': a.release_seconds,
    ...
    def resources(label):
        ...
        row['completed_cycles'] = len(result['cycles'])
        logs = row['logging']
        assert logs['disk_usage']['rc'] == 0 and logs['health']['rc'] == 0, 'journald unhealthy'
        assert all(v['size'] == 0 for v in logs['nginx_logs'].values()), 'nginx regular log grew'
        assert logs['directories']['/var/log/journal']['allocated_bytes'] == 0, 'persistent journal active'
        return row
    ...
    try:
        target('logging-policy-before', (HERE/'logging-policy-audit.py').read_text())
        ...
        run('load-source', ssh+['sudo -n tee /run/m8f2-logging-load.py'], (HERE/'logging-load.py').read_text())
        load = spawn('retention-load', ssh+['sudo -n systemd-run --wait --collect --unit=m8f2-logging-load --property=RuntimeMaxSec=7500 python3 /run/m8f2-logging-load.py'])
        children.insert(0, load)
        result['fixed_load'] = {'messages_per_second':32,'payload_hex_chars':384,'admin_sessions_per_minute':10}
        resources('baseline')
        schedule = []
        result['schedule'] = schedule.copy()
        next_sample = start+60; next_cycle = start; next_admin = start; admin_count = 0; index = 0
        while time.monotonic()-start < seconds:
            ...
            if now >= next_admin:
                run(f'admin-{admin_count:05d}', ssh+['sudo -n true'])
                admin_count += 1; next_admin += 6
                result['admin_sessions'] = admin_count
            if now >= next_sample:
            ...
            time.sleep(.5)
        resources('final')
        assert not result['events'], 'unexpected lifecycle event'
        ...
        target('logging-policy-after', (HERE/'logging-policy-audit.py').read_text())
        ...
        try:
            run('stop-retention-load', ssh+['sudo -n systemctl stop m8f2-logging-load.service'])
        except Exception as cleanup_error:
            result['load_cleanup_error'] = str(cleanup_error)
        (root/'stop').touch()
