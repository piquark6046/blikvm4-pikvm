...
def main():
 ...
 with (D/'events.jsonl').open('x',buffering=1) as events:
  ...
  with (D/'ready.json').open('x') as ready:ready.write(json.dumps(dict(pid=os.getpid(),started_utc=stamp(),uart=str(P),baud=115200,format='8N1',passive=True,duration_seconds=600,hotplug_retry=True,early_firmware_capture='FULL_CHAIN_REQUIRED',qualification_credit=False,scope='P3-A attempt 03 cycle 1 continuous UART capture')))
  event('watcher_ready')
