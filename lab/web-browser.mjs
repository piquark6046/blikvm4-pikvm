#!/usr/bin/env node
// Build VM qualification through a pre-established SSH loopback tunnel.
...
const ca = fs.readFileSync(path.join(privateDir, 'server.crt'));
...
try {
    result.tls = await new Promise((resolve,reject) => {
        const socket = tls.connect({host:'127.0.0.1', port:8443, servername:'localhost', ca}, () => {
            const cert=socket.getPeerCertificate();
            const expected=new crypto.X509Certificate(ca).fingerprint256;
    ...
    // Certificate trust is independently verified above; browser has an isolated profile.
    ...
    const ui=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width:1440,height:1000}});
    ...
    result.rate_load={video_clients:1,browser_page:'authenticated index'};
    ...
    const args=['lab/stream-client.py','--seconds',String(seconds),'--out',path.join(output,'stream'),
                '--','curl','--config',cookieFile,'--cacert',path.join(privateDir,'server.crt'),
