#!/usr/bin/env python3
"""Conditional H5R2 functional preflights; no reboot, reflash or service repair."""
import json
import os
...
P=runpy.run_path(str(BASE/'input/p3-h5r2.py'))
...
def prerequisites():
    ...
    P['prior_replay']('minimal-001')
    require(read(BASE/'controller/minimal-inheritance-replay.json')['result']=='H5R2_RUNTIME_INHERITANCE_ACCEPTED','runtime inheritance replay missing')
...
def prepare():
    ...
    replacements={f'p3-h5r2-{label}-browser{suffix}':f'{label}-browser{suffix}'
...
def collect(label,run):
    ...
    original=read(Path('/var/lib/blikvm-p3-s0/fresh-boot/inventory/target.json'))
    ...
    S['collect'](inv,run/(label+'-msd-gate'),G,ssh)
    return inv
...
def functional(number):
    ...
    for i in range(1,number):
        ...
        require(ack['result']=='H5R2_FUNCTIONAL_ACCEPTED' and ack['leaf_archive_sha256']==read(BASE/'controller'/name/'result.json')['seal']['sha256'],'prior functional replay missing')
    ...
    print(json.dumps(r));require(r['result']!='FAILED','H5R2 stopped; no later preflight or P3 cycle')
