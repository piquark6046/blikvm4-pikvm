#!/usr/bin/env node
// Prospective H5R2 contract probe; the controller externally isolates networking.
const fs = require('node:fs');
const crypto = require('node:crypto');
const cp = require('node:child_process');
const path = require('node:path');
const assert = require('node:assert/strict');
const base = '/var/lib/blikvm-p3-h5r2';
...
(async()=>{
  ...
  try {
    ...
    const raw=cp.execFileSync('/usr/bin/python3',[base+'/input/p3-h5r2.py','worker'],
