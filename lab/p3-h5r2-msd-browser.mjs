...
async function stage(name,connected,action){
 ...
 publish(stem+'.ready',{name,connected,...value});await wait(path.join(process.env.P3_CONTROL,path.basename(stem))+'.ok');
 const check=JSON.parse(fs.readFileSync(path.join(process.env.P3_CONTROL,path.basename(stem))+'.ok'));assert.equal(check.result,'passed',JSON.stringify(check));
...
try {
 ...
 launchGate(out);browser=await chromium.launch({headless:false});
